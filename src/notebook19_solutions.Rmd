...
```{r, question-01}
# I didn't ask you to actually do the mutate part, but it makes
# it clear the final outcome of the vote
casino |>
  summarize(votes_yes = sum(votes_yes), votes_no = sum(votes_no)) |>
  mutate(percent_yes = votes_yes / (votes_yes + votes_no))  
```
...
precinct |>
  left_join(race, by = "pnum") |>
  ggplot() +
    geom_sf(aes(fill = pblack)) +
    scale_fill_distiller(
      palette = 9, guide = "legend", n.breaks = 10, direction = 1
    )
```
...
precinct |>
  ...
  ggplot() +
    geom_sf(aes(fill = percent)) +
    geom_sf(data = roads, linewidth = 0.25) +
    scale_fill_distiller(
      palette = 8, guide = "legend", n.breaks = 10, direction = 1
    )
```
...
```{r, question-05}
casino |>
  inner_join(race, by = "pnum") |>
  ggplot(aes(pblack, percent)) +
...
# I did not ask you to do it this way, but it would be better to
# weight the model by the population of each precinct, so that the
# model puts more importance on larger precincts rather than smaller
# ones; this can be done by setting `weights` equal to the population
# variable within the `lm()` function
model <- lm(percent ~ pblack, data = temp, weights = population)
...
# Same data and model as above; just adding here to make solution
...
precinct |>
  left_join(augment(model, newdata = temp), by = "pnum") |>
  ggplot() +
...
```{r, question-08}
casino_location_geo <- casino_location |>
...
precinct |>
  ...
  ggplot() +
    geom_sf(aes(fill = .resid)) +
    geom_sf(data = roads, size = 0.25) +
    geom_sf(data = casino_location_geo, color = "#83a598", size = 10) +
    scale_fill_steps2(limits = c(-50, 50), n.breaks=10)
```
...
precinct |>
  left_join(race, by = "pnum") |>
  mutate(pother = 100 - pwhite - pblack) |>
  ggplot() +
    geom_sf(aes(fill = pother)) +
    geom_sf(data = roads, size = 0.25) +
    geom_sf(data = casino_location_geo, color = "#83a598", size = 7) +
    scale_fill_distiller(
      palette = 9, guide = "legend", n.breaks = 10, direction = 1
    )
```
